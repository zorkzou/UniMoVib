...
subroutine RdQCDt(iinp,iout,idt0,idt1,idt2,ibmt,Intact,IOP)
 ...
 !        open $(hess); optional, but needed for HF hessians because the defaults ones in *.out are not correct.
 !          It can be FCMFINAL or FCM, but not FCM000.
 else if(IOP(1) == 5) then
   if(LEN_TRIM(geom) == 0) geom='GRD'
   call DFOpen(iout,idt1,Intact,.False.,'GEOM',geom)
   if(LEN_TRIM(hess) == 0) hess='FCMFINAL'
   call DFOpen(iout,idt2,Intact,.False.,'HESS',hess)
...
subroutine PrtCoord(iout,Infred,NAtm,AMass,ZA,XYZ,APT,ipyvibms,ixyz)
 ...
 if(Infred == 0 .or. sum(ip) == 0) then
   ...
   write(iout,"(//,' Cartesian coordinates (Angstrom)',/,1x,108('-'),/,3x, &
     'No.   Atom    ZA',17x,'X',13x,'Y',13x,'Z',16x,'Mass',15x,'IR charge',/,1x,108('-'))")
...
subroutine prtconv(iout,grslt)
 implicit real(kind=8) (a-h,o-z)
 parameter(tolx=2.0d-3, tolg=5.0d-4, tole=1.0d-6)
 dimension :: grslt(5)

 write(iout,"(/,' Convergence of gradients',/,1x,68('-'))")
 write(iout,"(13x,'Item',17x,'Value     Tolerance      Converged?',/,1x,68('-'))")
 nyes = 0
 if(grslt(1) <= tolx*1.6d0) then
   write(iout,"(9x,'Max step',8x,2f14.6,12x,'Yes')") grslt(1), tolx*1.6d0
   nyes = nyes + 1
 else
   write(iout,"(9x,'Max step',8x,2f14.6,13x,'No')")  grslt(1), tolx*1.6d0
 end if
 if(grslt(2) <= tolx) then
   write(iout,"(9x,'RMS step',8x,2f14.6,12x,'Yes')") grslt(2), tolx
   nyes = nyes + 1
 else
   write(iout,"(9x,'RMS step',8x,2f14.6,13x,'No')")  grslt(2), tolx
 end if
 if(grslt(3) <= tolg*1.6d0) then
   write(iout,"(9x,'Max grad',8x,2f14.6,12x,'Yes')") grslt(3), tolg*1.6d0
   nyes = nyes + 1
 else
   write(iout,"(9x,'Max grad',8x,2f14.6,13x,'No')")  grslt(3), tolg*1.6d0
 end if
 if(grslt(4) <= tolg) then
   write(iout,"(9x,'RMS grad',8x,2f14.6,12x,'Yes')") grslt(4), tolg
   nyes = nyes + 1
 else
 ...
 if(grslt(5) <= tole) then
   ...
   write(iout,"('     Expected d-E',8x,2es14.2,13x,'No')")  grslt(5), tole
 end if
 write(iout,"(1x,68('-'))")
 if(nyes /= 5) write(iout,"(/,' This geometry may be not a stationary point.')")
