...
!
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
subroutine RdData1(iinp,iout,idt0,idt1,idt2,ibmt,Intact,IOP,Infred,IRaman,IGrd,ifbdfchk,NAtm,tag,ctmp,AMass,ZA,XYZ,Grd,FFx,  &
  APT,DPol,Scr1,Scr2,Scr3,ScrD)
implicit real(kind=8) (a-h,o-z)
logical :: Intact, ifbdfchk, IfFXX
dimension :: IOP(*)
...
select case(IOP(1))
  ...
  case(-2) ! UniMoVib (ALM); the size of ScrD should be 9*NAtm3 at least
    call RdALMode(idt0,Intact,Infred,IRaman,IGrd,ifbdfchk,NAtm,ctmp,AMass,ZA,XYZ,Grd,FFx,APT,DPol,ScrD)
...
!
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
subroutine RdNAtmALM(ifchk,NAtm,ctmp)
 implicit real(kind=8) (a-h,o-z)
 character*100 :: ctmp
 ...
 do while(.true.)
   ...
   if(ctmp(1:1) == ' ' .or. ctmp(1:1) == '!') cycle

   call charl2u(ctmp)

   ! read natm
   if(index(ctmp,"NATM") == 1) then
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!
! Read data from UniMoVib (ALM) data file (Ver. 1.2)
!
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
subroutine RdALMode(ifchk,Intact,Infred,IRaman,IGrd,ifbdfchk,NAtm,ctmp,AMass,ZA,XYZ,Grd,FFx,APT,DPol,Scr)
 implicit real(kind=8) (a-h,o-z)
 ...
 ! saved by BDFOPT?
 read(ifchk,"(a100)",err=2000,end=2000) ctmp
 call charl2u(ctmp)
 ! UniMoVib data file generated by BDFOPT(Check)
 if(index(ctmp,"UNIMOVIB") > 0 .and. index(ctmp,"BDFOPT(CHECK)") > 0) ifbdfchk = .true.
 ...
 do while(.true.)
   ...
   ! read APT in a.u.
   else if(index(ctmp,"APT") == 1) then
     Infred = 1
     read(ifchk,*,err=2070,end=2070)(APT(i),i=1,NAtm9)

   else if(index(ctmp,"NOAPT") == 1) then
     Infred = 0
     APT = 0.d0

   ! read DPR in a.u.
   else if(index(ctmp,"DPR") == 1) then
     IRaman = 1
     if(index(ctmp,"DPRSQ") == 1) then
   ...
   else if(index(ctmp,"NODPR") == 1) then
     IRaman = 0
     DPol = 0.d0

   ! read END
   else if(index(ctmp,"END") == 1) then
     exit
 ...
 2000  call XError(Intact,"Please check TITLE of the UniMoVib data file!")
...
! in fact they are forces (negative of the gradients)
Grd(1:NAtm3) = -Grd(1:NAtm3)
IGrd= 1
...
!
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
subroutine RdCFourA(ifchk,igeom,tag,ctmp,Intact,Infred,IGrd,NAtm,AMass,ZA,XYZ,Grd,FFx,APT,Scr)
implicit real(kind=8) (a-h,o-z)
real(kind=8) :: AMass(*),ZA(*),XYZ(3,*),Grd(3,*),FFx(NAtm*3,*),APT(3,*),Scr(*)
character*100 :: ctmp
...
character*3 :: Elm
 ...
 write(igau,"('#p b3lyp/genecp iop(1/10=1) opt(ts,ReadFC,cartesian,noeigentest,nomicro)')")
