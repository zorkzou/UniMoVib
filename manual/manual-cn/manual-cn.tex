...
\vspace{1cm} \zihao{3}（1.3.1版）\vspace{30 mm} \\
...
%用实验振动频率校正Hessian矩阵，在\verb|$ExpFrq|输入组（参见第\ref{sec:inp-expfrq}节）之后提供。默认：\verb|.False.|
...
计算方法参见\textbf{J. Chem. Theory Comput.} 14, 2558 (2018)。
...
DPR
  ...
  若不提供 DPR数据，改用 NODPR）
GRD
  （3*NATM，直角坐标下的梯度数组；若不提供梯度，改用 NOGRD）
\end{lstlisting}\end{colorboxed}
目前\verb|DPR|数据和拉曼强度仅支持\textsc{Gaussian}产生的fchk文件。\verb|GRD|数据仅用于某些外部程序（如\textsc{LModeA}）。
